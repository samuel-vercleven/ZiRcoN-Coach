    ...
    def _reset_payload(self, rows: list[dict]) -> dict:
        ...
        for row in rows:
            ...
            metrics = [
                ...
                _metric("Production post-reset XP / JCS par min", f"{_number(row.get('post_player_xp_per_min'), '.0f')} / {_number(row.get('post_player_jungle_cs_per_min'), '.2f')}", "post_player_xp_per_min/post_player_jungle_cs_per_min"),
                _metric("Production relative post-reset Gold / XP / JCS", f"{_number(row.get('post_relative_gold_per_min'), '+.0f')} / {_number(row.get('post_relative_xp_per_min'), '+.0f')} / {_number(row.get('post_relative_jungle_cs_per_min'), '+.2f')}", "post_relative_*_per_min"),
                _metric("Timing objectif", _enum(row.get("objective_timing")), "objective_timing"),
                _metric("Production après reset vs historique", production, "reentry_score/reentry_label"),
            ]
            context = []
            if row.get("objective_timing") and row.get("objective_timing") != "NONE":
                context.append(f"timing objectif : {_enum(row.get('objective_timing'))} · précédent {row.get('previous_objective_kind') or '—'} ({_number(row.get('previous_objective_seconds'), '.0f', ' s')}) · suivant {row.get('next_objective_kind') or '—'} ({_number(row.get('next_objective_seconds'), '.0f', ' s')})")
            ...
            events.append({"title": f"Reset / shop à {_time(timestamp)}", "subtitle": f"{_enum(row.get('phase')).title()} · {_enum(row.get('reset_origin'))}", "status": "AVAILABLE", "severity": "INFO", "metrics": metrics, "context": context, "item_ids": item_ids,
                           "technical": [f"reset_origin={row.get('reset_origin')}", f"sequence_classification={row.get('sequence_classification')}", f"objective_timing={row.get('objective_timing')}", f"previous_objective={row.get('previous_objective_kind')} side={row.get('previous_objective_side')} seconds={row.get('previous_objective_seconds')}", f"next_objective={row.get('next_objective_kind')} side={row.get('next_objective_side')} seconds={row.get('next_objective_seconds')}", f"reentry_label={label}", f"reentry_reference={row.get('reentry_reference_scope')} N={ref_size}", f"frozen_pre_tempo_score={row.get('frozen_pre_tempo_score')}", f"frozen_post_tempo_score={row.get('frozen_post_tempo_score')}", f"frozen_tempo_score_change={row.get('frozen_tempo_score_change')}", "Le score de ré-entrée mesure une production observée après le proxy ; il ne prouve pas la qualité causale du reset."]})
