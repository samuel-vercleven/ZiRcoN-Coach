...
class SettingsPage(QWidget):
    ...
    def __init__(self, local: LocalDataService, settings: RuntimeSettingsService, sync: RiotSyncService, parent=None):
        ...
        actions = QHBoxLayout(); self.account_save = QPushButton("Save account + scope"); self.account_save.setObjectName("CompactButton"); self.validate = QPushButton("Validate"); self.validate.setObjectName("CompactButton"); self.save = QPushButton("Validate & save key"); self.save.setObjectName("PrimaryButton"); actions.addWidget(self.account_save); actions.addWidget(self.validate); actions.addWidget(self.save); actions.addStretch(); layout.addLayout(actions); self.message = QLabel(); self.message.setWordWrap(True); self.message.setObjectName("Muted"); layout.addWidget(self.message); root.addWidget(api)
        data = QFrame(); data.setObjectName("Card"); dl = QVBoxLayout(data); data_title = QLabel("Local data status"); data_title.setObjectName("SectionTitle"); dl.addWidget(data_title); self.data_form = QFormLayout(); self.fields = {name: QLabel() for name in ("Database", "DB available", "Loaded matches", "Cached timelines", "Analyzed matches", "Latest match", "Last sync", "API configured", "Backend")}
        for name, field in self.fields.items(): field.setWordWrap(True); self.data_form.addRow(name, field)
        dl.addLayout(self.data_form); root.addWidget(data); root.addStretch(); self.account_save.clicked.connect(self._save_account); self.validate.clicked.connect(lambda: self._start_validation(False)); self.save.clicked.connect(lambda: self._start_validation(True)); self.refresh()
    def _save_account(self):
        try:
            self.settings.save_identity(self.riot_id.text().strip(), int(self.scope.currentData()))
            self.message.setText("Account and sync scope saved locally.")
    ...
    def refresh(self):
        ...
        self.fields["Database"].setText(status.db_path); self.fields["DB available"].setText("Yes" if status.db_available else "No"); self.fields["Loaded matches"].setText(str(status.match_count)); self.fields["Cached timelines"].setText(str(status.timeline_count)); self.fields["Analyzed matches"].setText(str(status.analyzed_match_count)); self.fields["Latest match"].setText(status.latest_match_date); self.fields["Last sync"].setText(status.last_sync_at); self.fields["API configured"].setText(self.settings.masked_key()); self.fields["Backend"].setText("Frozen through Phase 2I • zero-gate owner semantics preserved")
