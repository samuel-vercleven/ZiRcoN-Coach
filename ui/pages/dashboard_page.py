...
class DashboardPage(QWidget):
    ...
    def __init__(self, service: LocalDataService, assets: AssetService, parent=None):
        ...
        self.hero = QFrame(); self.hero.setObjectName("HeroCard"); self.hero.setFixedHeight(112); hero = QHBoxLayout(self.hero); hero.setContentsMargins(18, 14, 18, 14); self.profile_icon = AssetIcon(assets, 64); hero.addWidget(self.profile_icon)
        ...
        queue = QLabel("SoloQ"); queue.setObjectName("StatusBadge"); queue.setProperty("tone", "support"); hero.addWidget(queue); self.layout.addWidget(self.hero)
        ...
        action_text = QVBoxLayout(); action_title = QLabel('Coaching du moment'); action_title.setObjectName('SectionTitle'); self.next_action = QLabel(); self.next_action.setObjectName('Muted'); self.next_action.setWordWrap(True); action_text.addWidget(action_title); action_text.addWidget(self.next_action); action.addLayout(action_text, 1)
        self.review_latest = QPushButton('Voir l’analyse'); self.review_latest.setObjectName('PrimaryButton'); self.review_latest.clicked.connect(self._open_latest); action.addWidget(self.review_latest); self.layout.addWidget(self.action_card)
    ...
    def refresh(self):
        ...
        for match in matches:
            card = MatchCard(match, self.assets, compositions.get(match.match_id), compact=True); card.opened.connect(self.open_match); self.match_layout.addWidget(card)
