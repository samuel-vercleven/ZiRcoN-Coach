...
class MatchCard(QFrame):
    ...
    def __init__(self, match: MatchSummaryViewModel, assets: AssetService, parent=None):
        ...
        root = QHBoxLayout(self); root.setContentsMargins(14, 11, 12, 11); root.setSpacing(14)
        ...
        result = QLabel(f"{'VICTOIRE' if match.result == 'WIN' else 'DÉFAITE'}  •  {match.position}"); result.setProperty("result", match.result.lower()); identity.addWidget(result); root.addLayout(identity, 2)
        for title, value in (("K / D / A", match.kda_text), ("CS / MIN", "—" if match.cs_per_min is None else f"{match.cs_per_min:.1f}"), ("DURÉE", f"{match.duration_seconds // 60}:{match.duration_seconds % 60:02d}")):
        ...
        button = QPushButton("Ouvrir"); button.setObjectName("CompactButton"); button.clicked.connect(lambda: self.opened.emit(match.match_id)); root.addWidget(button)
