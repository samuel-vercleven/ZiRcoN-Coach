...
def test_named_data_value_and_stat_evidence_remain_raw():
    node = _record()["calculation_nodes"][1]
...
def test_nested_children_and_zero_are_preserved():
    record = _record()
    assert len(record["calculation_nodes"]) == 2
...
def test_names_slots_provenance_and_non_execution_are_exact():
    ...
    assert all(node["interpretation_status"] == UNINTERPRETED_CALCULATION_CLASS for node in record["calculation_nodes"])
